<script lang="ts">
  ...
  interface Props {
    ...
    onSaveCallback?: (response: SaveStageResponse) => void;
  }
  ...
  let { tournamentId, stageId, initialData, onSaveCallback }: Props = $props();

  // svelte-ignore state_referenced_locally
  let data = $state(
    initialData
      ? structuredClone($state.snapshot(initialData))
      : new StageData(),
  );
  ...
  async function submitStage(e: SubmitEvent) {
    ...
    try {
      ...
      if (onSaveCallback) {
        ...
        await goto(resolve(`/tournaments/${tournamentId}/organizer/rounds`));
      }
    } catch (err) {
      error =
    ...
    <div class="col-12">
      ...
      <form onsubmit={submitStage}>
        <fieldset disabled={isSubmitting}>
          ...
          {/if}
          {#each data.stage.table_ranges as tableRange (tableRange)}
